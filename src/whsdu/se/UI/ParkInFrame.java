...
public class ParkInFrame extends JInternalFrame {
	...
	public ParkInFrame() {
		...
		ensurejbutton.setText("确认入校");//设置按钮文本
		c.gridx = 0;
		c.gridy = 5;
		c.gridwidth = 1;
		c.gridheight = 1;
	...
	class parkActionListener implements ActionListener {			// 添加确认入场按钮的事件监听器
		public void actionPerformed(final ActionEvent e) {
			...
			else if(user.getOverage()<3)
				JOptionPane.showMessageDialog(null, "余额不足3元，请先充值再入校！");
			else{
				int i=Dal.parkin(a1,a2,a3,a4);
				if(i==1)
					JOptionPane.showMessageDialog(null, "入校成功！");	
				else
					JOptionPane.showMessageDialog(null, "入校失败！");	
			}
		}
	}
}
